...
public extension CGSize {
    func fitted(_ size: CGSize) -> CGSize {
        ...
        if fittedSize.width > size.width {
            fittedSize = CGSize(width: size.width, height: floor((fittedSize.height * size.width / CGFloat.maximum(fittedSize.width, 1.0))))
        }
        if fittedSize.height > size.height {
            fittedSize = CGSize(width: floor((fittedSize.width * size.height / CGFloat.maximum(fittedSize.height, 1.0))), height: size.height)
        }
        return fittedSize
    }
    ...
    func aspectFilled(_ size: CGSize) -> CGSize {
        let scale = CGFloat.maximum(size.width / CGFloat.maximum(1.0, self.width), size.height / CGFloat.maximum(1.0, self.height))
        return CGSize(width: floor(self.width * scale), height: floor(self.height * scale))
    }
    
    func aspectFitted(_ size: CGSize) -> CGSize {
        let scale = min(size.width / CGFloat.maximum(1.0, self.width), size.height / CGFloat.maximum(1.0, self.height))
        return CGSize(width: floor(self.width * scale), height: floor(self.height * scale))
    }
    
    func aspectFittedOrSmaller(_ size: CGSize) -> CGSize {
        let scale = min(1.0, min(size.width / CGFloat.maximum(1.0, self.width), size.height / CGFloat.maximum(1.0, self.height)))
        return CGSize(width: floor(self.width * scale), height: floor(self.height * scale))
    }
    ...
    func fittedToWidthOrSmaller(_ width: CGFloat) -> CGSize {
        let scale = min(1.0, width / CGFloat.maximum(1.0, self.width))
        return CGSize(width: floor(self.width * scale), height: floor(self.height * scale))
    }
