...
/**
 * `SKToastView`默认使用的contentView。其结构是：customView->textLabel->detailTextLabel等三个view依次往下排列。其中customView可以赋值任意的UIView或者自定义的view。
 *
 * @TODO: 增加多种类型的progressView的支持。
 */
open class SKToastContentView: UIView {
    /**
     ...
     */
    public var customViewMarginBottom: CGFloat = 8 {
        didSet {
            setNeedsLayout()
        }
    }
     ...
     */
    public var textLabelMarginBottom: CGFloat = 4 {
        didSet {
            setNeedsLayout()
        }
    }
     ...
     */
    public var detailTextLabelMarginBottom: CGFloat = 0 {
        didSet {
            setNeedsLayout()
        }
    }
     ...
     */
    public var textLabelAttributes = [NSAttributedString.Key.font: UIFontBoldMake(16), NSAttributedString.Key.foregroundColor: UIColor.white, NSAttributedString.Key.paragraphStyle: NSMutableParagraphStyle(lineHeight: 22)] {
        didSet {
            if textLabelText.count > 0 {
                // 刷新label的Attributes
     ...
     */
    public var detailTextLabelAttributes = [NSAttributedString.Key.font: UIFontBoldMake(12), NSAttributedString.Key.foregroundColor: UIColor.white, NSAttributedString.Key.paragraphStyle: NSMutableParagraphStyle(lineHeight: 18)] {
        didSet {
            if detailTextLabelText.count > 0 {
                // 刷新label的Attributes
    ...
    override open func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        let hasTextLabel = textLabel.text?.count ?? 0 > 0
        let hasDetailTextLabel = detailTextLabel.text?.count ?? 0 > 0
